...
const ANGULAR_INSTALL_SKIP_MAJOR = 22;
...
/**
 * Reads the Angular major version from the target project's package.json.
 * @param tree The virtual file system tree
 * @returns The Angular major version or null if unavailable
 */
function getAngularMajorFromPackageJson(tree: Tree): number | null {
  ...
  try {
    const packageJson = JSON.parse(packageJsonBuffer.toString('utf-8')) as {
...
function setupEslint(workspace: Workspace, context: SchematicContext, tree: Tree): Tree {
  ...
  addDevPackageToPackageJson(tree, '@angular-eslint/eslint-plugin', '^22.0.0');
  addDevPackageToPackageJson(tree, '@angular-eslint/eslint-plugin-template', '^22.0.0');
  addDevPackageToPackageJson(tree, '@angular-eslint/template-parser', '^22.0.0');
...
export function ngAdd(options: Schema): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    const angularMajor = getAngularMajorFromPackageJson(tree);
    ...
    addPackageToPackageJson(tree, '@angular/common', '^22.0.0');
    addPackageToPackageJson(tree, '@angular/core', '^22.0.0');
    ...
    if (angularMajor !== null && angularMajor >= ANGULAR_INSTALL_SKIP_MAJOR) {
      context.logger.warn(
        '⚠ Skipping automatic package install for Angular 22+: PrimeNG 21 peer dependencies can cause npm ERESOLVE.'
      );
      context.logger.warn(
        '⚠ Run `npm install --legacy-peer-deps` manually in the target project after schematic completion.'
      );
    } else {
