...
describe('App Routing Configuration', () => {
  ...
  it('should load ModalarmePatternsComponent with title and auth guard', async () => {
    const route = routes.find((r) => r.path === 'modalarme-patterns');
    expect(route).toBeDefined();
    expect(route!.data?.title).toBe('isyAngularWidgetsDemo.websiteTitles.modalarmePatterns');
    expect(route!.canActivate).toContain(canActivateAuth);
    expect(typeof route!.loadComponent).toBe('function');
