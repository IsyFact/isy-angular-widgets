...
> **Dieser Branch gehört zur Versionslinie 21 (Angular 21).** Beiträge, die ausschließlich diese Linie betreffen, werden gegen `develop-21` eingereicht.
...
```text
feat!: IFS-5174 drop moment.js from date validation

BREAKING CHANGE: moment.js ist keine (Peer-)Abhängigkeit mehr.
Bestehende Anwendungen sollten Date-Objekte oder ISO-Strings übergeben.
```
...
Releases werden über einen Branch nach dem Schema `release/<planned-version>` vorbereitet, zum Beispiel `release/21.3.0`.
...
Weitere Details zum Vorgehen stehen im Abschnitt *Releases erstellen* der [README.md](README.md).
